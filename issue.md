...
- 你得首先…… 使用本地终端 输入命令： sudo apt-get install git ，安装git
...
- 怎么办，不着急——使用本地终端SSH 命令 ：ssh-keygen -C "yourname@yourcompany.com" -f ~/.ssh/github ，它能生成sshkey，被保存在~/.ssh/github.pub 里。
